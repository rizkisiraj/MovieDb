...
const CrewCastCard = ({ person, type }: { person:Cast, type:'cast' | 'crew'  }) => {
  return (
    <HStack padding="16px" textAlign="center" height="100%">
      <Avatar size="xl" mb="8px" src={API_ENDPOINT.GET_MOVIE_IMAGE(person.profile_path)} />
      <Box>
        <Text fontWeight="bold" color="teal">{person.name}</Text>
        <Text fontSize="sm" color="teal">{type === 'cast' ? person.character : person.job}</Text>
      </Box>
   </HStack>
  )
} 
